...
typealias loginCompletedClosure = (_ isCancelled: Bool, _ error: Error?) -> ()
typealias loggedOutClosure = () -> ()

struct FacebookConnector {
    
    private var loginButtonDelegate: LoginButtonDelegateObject?
    
    mutating func loginButton(completedCallback: @escaping loginCompletedClosure, loggedOutCallback: @escaping loggedOutClosure) -> FBSDKLoginButton {
        let loginButton = FBSDKLoginButton()
        loginButton.readPermissions = Permissions.read
        
        self.loginButtonDelegate = LoginButtonDelegateObject(completedCallback: completedCallback, loggedOutCallback: loggedOutCallback)
        loginButton.delegate = self.loginButtonDelegate
        
        return loginButton
    }
    
    func startGraphRequestConnection(completionHandler: @escaping ([String: AnyObject]?, Error?) -> ()) {
        
        let request = FBSDKGraphRequest(graphPath: "me", parameters: ["fields": Properties.User.selected.joined(separator: ",")])
...
private class LoginButtonDelegateObject: NSObject, FBSDKLoginButtonDelegate {
    
    let completed: loginCompletedClosure
    let loggedOut: loggedOutClosure
    
    init(completedCallback: @escaping loginCompletedClosure, loggedOutCallback: @escaping loggedOutClosure) {
        completed = completedCallback
        loggedOut = loggedOutCallback
    }
